...
def scrape_satu(page, no_perkara):
    ...
    try:
        ...
        if "Just a moment" in page.title() or "Cloudflare" in page.title():
            print(f"    [!] Cloudflare terdeteksi, menunggu...")
        ...
        try:
            page.wait_for_selector(".widget-content table", timeout=15000)
        except Exception:
            pass
        ...
        lewat = []
        depan = []
        tabel_container = soup.select_one(".widget-content table")
        if tabel_container:
            for baris in tabel_container.find_all("tr")[1:]:
                kolom = baris.find_all(["td", "th"])
                if len(kolom) >= 2:
                    if len(kolom) >= 3:
                        tgl_teks  = kolom[1].get_text(separator=" ", strip=True)
                        agenda_teks = kolom[2].get_text(separator=" ", strip=True)
                    else:
                        tgl_teks  = kolom[0].get_text(separator=" ", strip=True)
                        agenda_teks = kolom[1].get_text(separator=" ", strip=True)
                    try:
                        tgl_obj = datetime.strptime(tgl_teks[:10], "%d-%m-%Y").date()
                        if tgl_obj <= hari_ini:
                            lewat.append({"tgl": tgl_teks, "agenda": agenda_teks})
                        else:
                            depan.append({"tgl": tgl_teks, "agenda": agenda_teks})
                    except ValueError:
                        pass
        ...
        if not depan:
            ...
            for st in soup.select(".bs-wizard-step"):
                ...
                try:
                    no = int(dot.get("data-step")) if dot and dot.get("data-step") else None
                except (TypeError, ValueError):
                    no = None
                h5 = st.find("h5")
                tgl_step = h5.text.strip() if h5 else ""
                steps.append({"step_no": no, "status": status, "tanggal": tgl_step})
...
def jalankan_scraper():
    ...
    daftar_id = [p["id"] for p in data_perkara["perkara"]]
    print(f"Ditemukan {len(daftar_id)} perkara untuk di-scrape.\n")
    ...
    with sync_playwright() as p:
        ...
        for no_perkara in daftar_id:
            ...
            print(f"  Terakhir    : [{hasil['tgl_terakhir']}] {hasil['agenda_terakhir']}")
            print(f"  Selanjutnya : [{hasil['tgl_selanjutnya']}] {hasil['agenda_selanjutnya']}\n")

            jeda = random.uniform(5.5, 9.5)
